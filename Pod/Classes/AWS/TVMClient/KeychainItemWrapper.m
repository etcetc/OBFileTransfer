/*
 ...
 This shitty ass code comes from Apple
 */
...
- (id)initWithIdentifier: (NSString *)identifier accessGroup:(NSString *) accessGroup;
{
    if (self = [super init])
    {
        ...
        //        NOTE:
        //        The keychain uses the combination of kSecAttrService and kSecAccount to make a unique ID
        //        so if the user wants to save an AccntID and it's the same as one for a different service, you're screwed - the
        //        thing will fail w/ an InternalConsistencyError.  So alhtough the examples save the key under kSecAttrGeneric
        //        I'm setting it under account to make sure it's unique
...
//        [genericPasswordQuery setObject:identifier forKey:(__bridge id)kSecAttrService];
        ...
        [genericPasswordQuery setObject:(id) kCFBooleanTrue forKey:(__bridge id)kSecReturnData];
        
        //        [genericPasswordQuery setObject:(__bridge id) kSecAttrAccessibleWhenUnlockedThisDeviceOnly forKey:(__bridge id)kSecAttrAccessible];
        
        NSDictionary *tempQuery = [NSDictionary dictionaryWithDictionary:genericPasswordQuery];
        
        CFMutableDictionaryRef outDictionary = NULL;
        
        //        Query the keychain to see if the item is still in there
        if (SecItemCopyMatching((__bridge CFDictionaryRef)tempQuery, (CFTypeRef *)&outDictionary) == noErr)
        {
            // load the saved data from Keychain and conver thte password to be a string
            
            keychainItemData = [NSMutableDictionary dictionaryWithDictionary:(__bridge id)outDictionary];
            NSData * data = [keychainItemData objectForKey:(__bridge id)kSecValueData];
            NSString *stringData = [[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:NSUTF8StringEncoding];
            [keychainItemData setObject:stringData forKey:(__bridge id)kSecValueData];
...
//            [keychainItemData setObject:identifier forKey:(__bridge id)kSecAttrService];
            if (accessGroup != nil)
            {
#if TARGET_IPHONE_SIMULATOR
                // Ignore the access group if running on the iPhone simulator.
...
// Delete the item in the keychain
-(void) removeFromKeychain
{
    if (keychainItemData)
    {
        ...
        NSAssert( oserror == noErr || oserror == errSecItemNotFound, @"Problem deleting current dictionary." );
    }
}

// This converts a normal dictionary to a format that is good for saving to the keyhain
// All it really does is force the class to be "generic Password" format, and fomats the
// Object to be UTF encoded
- (NSMutableDictionary *)dictionaryToSecItemFormat:(NSDictionary *)dictionaryToConvert
{
...
// Going from sec format to our standard dictionary, we force the class
// to be generic password and indicate that return data must be included, then we actually
// search to find the items again
- (NSMutableDictionary *)secItemFormatToDictionary:(NSDictionary *)dictionaryToConvert
{
    // The assumption is that this method will be called with a properly populated dictionary
    ...
    // Indicate that we dont' need a dictionary back, just the generic data value, then convert it to a string
    //    and put it into the dictionary
